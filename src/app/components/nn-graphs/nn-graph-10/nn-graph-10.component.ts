...
export class NnGraph10Component implements OnInit, OnDestroy {
  ...
  @Input() easeType = easingFunctions['linear']; // controls acceleration/deceleration of motion
  @Input() linkPulseScale = 4; // control how much the line stroke-width and opacity change depending on activation difference.
  @Input() linkPulseOpacity = 0.7; // control how much the line stroke-width and opacity change depending on activation difference.
  @Input() pulseDuration = 700; // how long a pulse (circle) takes to travel from start to end of a link.
  @Input() pulseSpacing = 200;  // how long to wait before the next pulse starts
  ...
  private updateGraph(activations: number[][]): void {
    ...
    if (this.passDirection !== 'none') {
      ...
      pulseMerge.each((d, i, nodes) => {
        ...
        // forward → determines which direction the pulse travels.
        ...
        // act → current activation value; only affects circle size (r) and opacity, not frequency.
        ...
        const travel = () => {
          circle
            .attr('cx', xStart)
            .attr('cy', yStart)
            .attr('r', 3 + 4 * act)           // <- size depends on activation
            .attr('opacity', 0.6 + 0.4 * act) // <- opacity depends on activation
            .attr('fill', pulseColor)
            .transition()
            .duration(this.pulseDuration)     // <- travel time controlled here
            .ease(this.easeType)              // <- easing controls acceleration
            .attr('cx', xEnd)
            .attr('cy', yEnd)
            .attr('opacity', 0)
            .on('end', () => setTimeout(travel, this.pulseSpacing)); // <- loop again after pulseSpacing ms
        };
