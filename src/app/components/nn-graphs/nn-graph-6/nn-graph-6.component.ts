...
export class NnGraph6Component implements OnInit, OnDestroy {
  @ViewChild('svgRef', { static: true }) svgRef!: ElementRef<SVGSVGElement>;

  @Input() easeDuration = 600;          // ms
  @Input() easeType = d3.easeCubicInOut;
  @Input() haloRadius = 12;             // px
  ...
  private updateGraph(activations: number[][]): void {
    ...
    linkSel.join(
      enter => enter.append('line')
        ...
        .attr('stroke-width', 1)
        .attr('opacity', 0)
        .call(enter => enter.transition().duration(this.easeDuration).ease(this.easeType)
          .attr('opacity', d => Math.abs(d.source.activation - d.target.activation))
          .attr('stroke-width', d => 1 + 4 * Math.abs(d.source.activation - d.target.activation))
        ),
      update => update.transition().duration(this.easeDuration).ease(this.easeType)
        .attr('opacity', d => Math.abs(d.source.activation - d.target.activation))
        .attr('stroke-width', d => 1 + 4 * Math.abs(d.source.activation - d.target.activation)),
      exit => exit.remove()
    );
